...
function [X_hat] = RobustParticleFiltering(Y,X0,F,G,Q,R,x0_hat,P0)
    % This function realizes the Distributionally Robust Particle Filter
    ...
    for k = 1:SimulationStep
        ...
        % Fight Against Process Model Uncertainty
        if false                            % You may set this to true.
            u = MaxEntDistDiscreteKL(u');
            u = u';
        end
        ...
        for i = 1:N_particle
            MU = [
                sqrt((x(1,i) - X0(1,k))^2 + (x(3,i) - X0(2,k))^2)
                % no difference between "atan" and "atan2" for this example
                atan((x(3,i) - X0(2,k))/(x(1,i) - X0(1,k)))
            ];
            SIGMA = R;
            % Fight Against Measurement Model Uncertainty
            if true
                SIGMA = 5*SIGMA;
            end
        ...
        if max(likelihood) > 1e-6       % Outlier Identification and Rejection
